taylor.diagram_mod<- function (ref, model, add = FALSE, col = "red", pch = 19, pos.cor = TRUE, 
                               ...
                               mar = c(4, 3, 4, 3), x_axis_range=1, ...) 
{
  ...
  if (is.list(model)) 
    ref <- unlist(model)
  SD <- function(x, subn) {# function to get standatd deviation 
  ...
  #sd.r <- SD(ref, subn) #changed SD to the R function
  sd.r<-sd(ref)
  #sd.f <- SD(model, subn)
  sd.f<-sd(model)
  if (normalize) {
  ...
  maxsd <- x_axis_range * max(sd.f, sd.r) # modify this when adjusting x axis 
  oldpar <- par("mar", "xpd", "xaxs", "yaxs")
  if (!add) {
    par(mar = mar)
    if (pos.cor) { ## positive correlation starts here
      if (nchar(ylab) == 0) 
      ...
      segments(cos(smltickangles) * maxsd, sin(smltickangles) * 
                 ...
                 0.99 * maxsd)
    }# positive correlation ends here
    else {
      x <- ref
      y <- model
      ...
      Eprime <- sqrt(sum(sumofsquares)/length(complete.cases(x))) ## THIS IS THE CRMSE following Taylor (2001) but it is not used for anything, same as all the variables in lines 119-121
      E2 <- E^2 + Eprime^2
      RMSE <- sqrt(mean((x - y)^2)) # this is the RMSE calculation, but it is not part of the plot or the original function, Marco added this 
      if (add == FALSE) {
        maxray <- x_axis_range * max(sd.f, sd.r) # modify this for x axis
        ...
        discrete <- seq(180, 0, by = -1)#from here to line 142, the script draws the semicircle. discrete is just the sequence from 0 to 180
        listepoints <- NULL
        for (i in discrete) {
        ...
        for (i in grad.corr.lines) { #from here to lines 154, it draws correlation lines
          lines(c(0, maxray * i), c(0, maxray * sqrt(1 - i^2)), lty = 3, lwd= 1.5)
          lines(c(0, -maxray * i), c(0, maxray * sqrt(1 - i^2)), lty = 3,lwd= 1.5)
        }
        for (i in grad.corr.full) {
          text(1.05 * maxray * i, 1.05 * maxray * sqrt(1 - i^2), i, cex = cex.axis, adj = cos(i)/2)
          text(-1.05 * maxray * i, 1.05 * maxray * sqrt(1 - i^2), -i, cex = cex.axis, adj = 1 - cos(i)/2)
        }
        seq.sd <- seq.int(0, 2 * maxray, by = (maxray/5))[-1] # CRMSE LINES ARE DRAWN FROM l151-163, what confuses me is that this is all done using the seq.sd, which is a sequence created based on maxray, which is maxray <- 1 * max(sd.f, sd.r), so it's based on standard deviation, not on Eprime (line)   
        #seq.RMSE<- seq.int(0, 2* RMSE, by = (RMSE/5))[-1]# added sequence of RMSE, tried to pass this to the points function in line 158 but it was a mess... 
        for (i in 1:length(seq.sd)) {
          xcircle <- sd.r + (cos(discrete * pi/180) * seq.sd[i])
          ycircle <- sin(discrete * pi/180) * seq.sd[i]
          for (j in 1:length(xcircle)) {
            if ((xcircle[j]^2 + ycircle[j]^2) < (maxray^2)) {
              points(xcircle[j], ycircle[j], col = "darkgreen", pch = ".", cex = 1.5)
              if (j == 10) 
                text(xcircle[j], ycircle[j], signif(seq.sd[i], 2), cex = 0.9, col = "darkgreen", srt = 90) # numbers are now RMSE, this was modified
            }
          }
        }
        ...
        for (i in seq.sd) {
          ...
          if (i) 
            lines(xcircle, ycircle, lty = 3, col = "blue", cex=1, lwd= 1.5)
          text(min(xcircle), -0.06 * maxray, signif(i, 2), cex = cex.axis, col = "blue")
          text(max(xcircle), -0.06 * maxray, signif(i, 2), cex = cex.axis, col = "blue")
        }
  ...
  points(sd.f * R, sd.f * sin(acos(R)), pch = pch, col = col, cex = pcex) # this is the part where the correlation coefficient and the standard deviation are drawn in the plot
  invisible(oldpar)
}
